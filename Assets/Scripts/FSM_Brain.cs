...
public class FSM_Brain : MonoBehaviour
{
    ...
    {
        Collider[] playerColls = Physics.OverlapSphere(transform.position, detectionRadius, enemyMask); //uses Physics.OverlapSphere to detect all colliders within a sphere of radius detectionRadius centered at the object's position (transform.position).
                                                                                                        //Only colliders on the layers specified by enemyMask are considered. The results are stored in an array of Collider objects (playerColls).
